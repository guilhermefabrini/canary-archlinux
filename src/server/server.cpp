...
void ServiceManager::stop() {
	...
	for (auto &servicePortIt : acceptors) {
		try {
			asio::post(io_context, [servicePort = servicePortIt.second] {
...
void ServicePort::open(uint16_t port) {
	...
	try {
		if (g_configManager().getBoolean(BIND_ONLY_GLOBAL_ADDRESS)) {
			acceptor = std::make_unique<asio::ip::tcp::acceptor>(io_context, asio::ip::tcp::endpoint(asio::ip::address(asio::ip::make_address_v4(g_configManager().getString(IP))), serverPort));
		} else {
